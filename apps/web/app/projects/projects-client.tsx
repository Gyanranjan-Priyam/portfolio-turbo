...
export function ProjectsClient({ projects }: ProjectsClientProps) {
  ...
  const projectsPerPage =
    viewMode === "grid" ? PROJECTS_PER_PAGE_GRID : PROJECTS_PER_PAGE_LIST;
  ...
  // "Ctrl+K" or "/" to open, Escape to close
  useEffect(() => {
  ...
  // Lock body scroll & pause Lenis when search is open
  useEffect(() => {
  ...
  return (
    <div className="py-8 sm:py-12">
        ...
        <div className="flex items-start justify-between mb-10">
          <div>
            ...
            <p className="text-xs sm:text-sm font-mono text-muted-foreground leading-relaxed">
              ...
              tools. You can also checkout my <Link
                href="/templates"
                className="font-bold hover:text-foreground"
              >
                Templates
              </Link> section for some helpful templates.
            </p>
          </div>
          <div className="flex items-center gap-2 shrink-0 ml-4 mt-1">
            <button
              ...
              className="flex items-center gap-1.5 rounded-full border p-2 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground cursor-pointer"
            >
      ...
      {viewMode === "grid" ? (
        <div className="grid gap-4 mt-8 sm:grid-cols-2 items-stretch">
          {paginatedProjects.map((project, i) => (
            <BlurFade key={project.id} delay={0.12 + i * 0.05} inView className="h-full">
              <Link
        ...
        {isSearchOpen && (
          <>
            ...
            <motion.div
              ...
              data-lenis-prevent
              className="fixed inset-x-0 top-[10vh] z-50 mx-auto w-full max-w-xl px-4 overscroll-contain"
            >
              <div className="overflow-hidden rounded-sm border bg-card shadow-2xl">
                {/* Search Input */}
                <div
                  className={`flex items-center gap-3 px-4 py-3${hasActivity ? " border-b" : ""}`}
                >
                  ...
                  <kbd className="hidden shrink-0 items-center gap-1 rounded border bg-muted px-1.5 py-0.5 text-[10px] font-mono font-medium text-muted-foreground sm:inline-flex">
                    ESC
                  </kbd>
                </div>
                        ...
                        <AnimatePresence mode="popLayout">
                          {filteredProjects.length > 0 ? (
                              ...
                              >
                                ...
                                >
                                  <div className="flex items-start justify-between gap-3">
                                    <h3 className="text-sm font-bold font-sans tracking-tight text-foreground uppercase group-hover:underline">
                                      <HighlightText
                                        text={project.title}
                                  ...
                                  <p className="mt-1 line-clamp-2 text-xs font-mono leading-relaxed tracking-tight text-muted-foreground">
                                    <HighlightText
                                      text={project.desc[0]}
                                      query={debouncedQuery}
                                    />
                                  ...
                                  <div className="mt-2 flex flex-wrap gap-1">
                                    {project.tech.slice(0, 6).map((tech) => (
                                      <span
                                        key={tech}
                                        className="rounded-full border border-border/70 bg-muted/20 px-2 py-0.5 text-[10px] font-mono text-muted-foreground"
                                      >
                                        <HighlightText
                                          text={tech}
                                          query={debouncedQuery}
                                    ...
                                    {project.tech.length > 6 && (
                                      <span className="rounded-full border border-border/70 bg-muted/20 px-2 py-0.5 text-[10px] font-mono text-muted-foreground">
                                        +{project.tech.length - 6} more
                                      </span>
                                    )}
                                  </div>
                            ...
                            >
                              ...
                              <button
                                onClick={clearSearch}
                                className="mt-3 rounded-lg bg-muted px-4 py-1.5 text-xs font-mono font-medium text-foreground transition-colors hover:bg-muted/80"
                              >
                                Clear search
                              </button>
                            </motion.div>
